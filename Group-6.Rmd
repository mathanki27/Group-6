...
#Summaries the length of coding sequences
summary(cds)
```
**code explanation**
...
summary(cds)- this function is used to which is used to summaries various details of the sequences including the length, mode, classes etc.
...
The next part to question three is to create boxplot for the coding sequence length in both the organisms
Create the boxplot for E.coli
```{r}
#Read the fasta file of E.coli
cds <- seqinr::read.fasta("ecoli_cds.fa")
...
#Generate the boxplot for sequence length of ecoli
boxplot(len,ylab="sequence length (bp) for E.coli")
```
**code explanation**
cds <- seqinr::read.fasta("ecoli_cds.fa")- this code is used to convert the fasta file containing coding sequences of E.coli in readable R format and then save it as a vector called cds.

len <- as.numeric(summary(cds)[,1])- This function is used to summarise the first column of the data and changes it into numerical vector under "len".

Boxplot function is used to generate the graph for the sequence length for E.coli.


Create boxplot for bifidobacterium
```{r}
#Read the fasta file of Bifidobacterium
cds1 <- seqinr::read.fasta("bifido_cds.fa")
...
#Generate the boxplot for sequence length of bifidobacterium
boxplot(len,ylab="sequence length (bp) for bifidobacterium")
```
**code explanation**
similar code chunk is used to create boxplot for the bifidobacterium sequences which involves reading the fasta file into R, summarizing the first column as vector and generate the graph.


The next part to question 3 talks about finding the mean and median values of coding sequence length
```{r}
#Read the fasta file of E.coli
cds <- seqinr::read.fasta("ecoli_cds.fa")
...
Difference1
```{r}
#Find GC content first first sequence of E.coli
GC(cds[[1]])
```
**code explanation**
The code above is used to calculate the GC  content of sequence 1 of E.coli, which is done with the help of the function GC which tries to fing the number of GC in sequence 1, which is obtained as 0.5151515.

```{r}
#find GC content in sequence 1 of bifidobaterium
GC(cds1[[1]])
```
**code explanation**
The code above is used to obtain the GC content in sequence 1 of the bifidobacterium which is obtained to be 0.4494624.Thus it is noted that E.coli sequence 1 contains more of GC content as compared to that of bifidobacterium.

difference2
```{r}
#Find the total GC content for E.coli
GC(unlist(cds[1:4239]))
```
**code explanation**
The code above is used to obtain GC content for all the sequences in E.coli, whic is 0.5193114

```{r}
#Find the total GC content for bifidobacterium
GC(unlist(cds1[1:1191]))
```
**code explanation**
The code above is used to obtain the GC content for all sequences in bifidobacterium 0.4427088.this shows that E.coli has more GC content in all sequences when compared to that of bifidobacterium.

Difference 3 
```{r}
#Count number of each base in sequence 1 of E.coli
count(cds[[1]],1)
```
**code explanation**
The code above is used to count each of the bases (A,T,G,C) in the sequence 1 of E.coli bacterium which is obtained to be as A-21, C-22, G-12, t-11.

```{r}
#Count number of each base in sequence 1 of bifidobacterium
count(cds1[[1]],1)
```
**code explanation**
The code given above is used to obtain the A,T,G,C content in the first sequence of bifidobacterium, which is obtained to be A-112, c-85, G-124, T-144, Thus can be interpreted that bifidobaterium has more of each base in its first sequence.


difference 4
Apart from these differences ,there are other differences in the sequences of E.coli and bifidobacterium too which include the difference in mean values of the sequence length for both organisms(938.5534,937.1889),difference in the median of the length of sequences of both organisms(831, 795) , the number of sequences(4239,1191), sum of the length of sequences of both bacterium (3978528,1116192) respectively.
...
# Unlist the DNA sequences (combine all sequences into one long sequence)
dna_combined <- unlist(cds1)
dna_combined2 <- unlist(cds2)
```
...
dna_counts <- table(dna_combined)
dna_counts2 <- table(dna_combined2)
```
**Code Explanation**
The table() function counts the occurences of each nucleotide and displays the results in the form of a tabular column

```{r}
# Calculate the total number of nucleotides
total_bases <- sum(dna_counts)
total_bases2 <- sum(dna_counts2)
```
**Code Explanation**
The sum() function takes all the counts from the dna_counts tabular column and adds them together.total_bases indicates the variable it is stored in.In a similar way The sum() function takes all the counts from the dna_counts 2 tabular column and adds them together total_bases2 indicates the variable it is stored in

```{r}
# Calculate the frequency (proportion) of each nucleotide
dna_frequency <- dna_counts / total_bases
dna_frequency2 <- dna_counts2 / total_bases2
```
**Code Explanation**
dna_counts / total_bases calculates the frequency of each nucleotide in the first sequence and stores this information in dna_frequency variable.Similarly dna_counts2 / total_bases calculates the frequency of each nucleotide in the second sequence and stores this information in dna_frequency2 variable.

#Creation of barplot for nucleotide and amino acide frequency
```{r}
barplot(dna_frequency,xlab="nucleotides",ylab="frequency", main="Bifido CDS composition")
barplot(dna_frequency2,xlab="nucleotides",ylab="frequency", main="E coli CDS composition")
```
**Code Explanation**
The barplot function is used to create bar charts.Using the function barplot(dna_frequency) a barplot is created for the nucleotide sequences in "Bifido CDS composition",xlab labels the x axis as "nucleotides" and ylab labels the y axis as "frequency".Using the function barplot(dna_frequency) a barplot is created for the nucleotide sequences in "E coli CDS composition",xlab labels the x axis as "nucleotides" and ylab labels the y axis as "frequency"

```{r}
#Count the occurrences of each amino acid for protein_combined1
aminoacid_count1 <- table(protein_combined1)
aminoacid_count2 <- table(protein_combined2)
```
**Code Explanation**
The table() function counts the number of unique amino acids present and represents the data in a tabular format.

```{r}
#Calculate the total number of amino acids in protein_combined1
total_amino_acids1 <- sum(aminoacid_count1)
total_amino_acids2 <- sum(aminoacid_count2)
```
**Code Explanation**
sum() calculates the total number of amino acids in the set of protein sequences.

```{r}
#Calculate the frequency (proportion) of each amino acid for protein_combined1
aa_frequency1 <-aminoacid_count1 / total_amino_acids1
aa_frequency1 <-aminoacid_count2 / total_amino_acids2
```
**Code Explanation**
aa_frequency1 <-aminoacid_count1 / total_amino_acids1 code divides the count of each amino acid in aminoacid_count1 by the total number of amino acids stored in total_amino_acids1
aa_frequency1 <-aminoacid_count2 / total_amino_acids2 code divides the count of each amino acid in aminoacid_count2 by the total number of amino acids stored in total_amino_acids2

```{r}
barplot(aa_frequency1,xlab="nucleotides",ylab="frequency", main="Bifido CDS composition")
barplot(aa_frequency2,xlab="nucleotides",ylab="frequency", main="E coli CDS composition")
```
**Code Explanation**
The barplot() function is used to create barcharts,xlab="nucleotides" denotes the name for the x axis as "nucleotides"ylab="frequency"denotes the name for the y axis as "frequency" and main denotes the name of the chart as "Bifido CDS composition" in the first chart and "E coli CDS composition" in the second chart 
# Differences between the two organisms
The Ecoli cds DNA composition was around the same for a,c and t values but was slightly higher for g values whereas in the Bifido cds DNA composition there was great variability between the a,c,g and t values.
Whereas in the amino acid frequencies in both Bifido and Ecoli there was not much variablity seen between the two.
...
```{r}
#To generate codon usage list of all sequences for E.coli
codon_usage_list <- lapply(cds, uco)

#To generate codon usage list of all sequences for bifidobacterium
codon_usage_list1 <- lapply(cds1, uco)
```
**code explanation**
The code chunk used above is to obtain the codon usage of of both the organisms. 
lapply() is used to apply a function throughout the data acting on each element in it . and in this case , it is used to apply the uco function to each element in the data cds/cds1 and store the generated data in codon_usage_list/codon_usage_list1.


Next step involves the generation of codon usage table .
```{r}
# Generate table for codon usage list of E.coli
 codon_usage_table <- do.call(rbind, codon_usage_list)

 # View the table for E.coli
 head(codon_usage_table)

 #generate table for codon usage list of bifidobacterium
 codon_usage_table1 <- do.call(rbind, codon_usage_list1)
 
 #View the table for bifidobacterium
 head(codon_usage_table1)

```
**code explanation**
The code chunk given above is used to create table for the codon_usage_list/ codon_usage_list 1 for E.coli and bifidobacterium respectively. 
do.call is a function which allows another function to run on all elements in the data which in this case is codon_usage_list/ codon_usage_list1.
rbind is a function that is used tobind the data into rows and thus syseatically arrange the data.
